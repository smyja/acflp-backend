...
async def get_user(user_id: int, db: Annotated[AsyncSession, Depends(async_get_db)]):
    """Get a user by ID."""
...
async def get_users(page: int = 1, items_per_page: int = 10, db: Annotated[AsyncSession, Depends(async_get_db)]):
    ...
    return paginated_response(crud_data=users, page=page, items_per_page=items_per_page)
```
...
async def create_user(user_data: UserCreate, db: Annotated[AsyncSession, Depends(async_get_db)]):
...
async def update_user(user_id: int, user_data: UserUpdate, db: Annotated[AsyncSession, Depends(async_get_db)]):
...
async def delete_user(user_id: int, db: Annotated[AsyncSession, Depends(async_get_db)]):
    if not await crud_users.exists(db=db, id=user_id):
        raise HTTPException(status_code=404, detail="User not found")
...
async def permanently_delete_user(user_id: int, db: Annotated[AsyncSession, Depends(async_get_db)]):
...
async def get_users(
    ...
    users = await crud_users.get_multi(db=db, offset=(page - 1) * limit, limit=limit)
    return users["data"]
```
...
```python
from app.core.exceptions.http_exceptions import NotFoundException, DuplicateValueException, ForbiddenException
