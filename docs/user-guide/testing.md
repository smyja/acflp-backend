...
async def test_post(async_session: AsyncSession, test_user: User) -> Post:
    """Create a test post."""
    post = Post(title=fake.sentence(), content=fake.text(), created_by_user_id=test_user.id)
...
async def superuser_headers(async_client: AsyncClient, test_superuser: User) -> dict:
    ...
    login_data = {"username": test_superuser.username, "password": "superpassword123"}
...
class TestUserModel:
    ...
    async def test_user_relationships(self, async_session, test_user):
        ...
        post = Post(title="Test Post", content="Test content", created_by_user_id=test_user.id)
...
class TestPostModel:
    ...
    async def test_post_creation(self, async_session, test_user):
        ...
        post = Post(title="Test Post", content="This is test content", created_by_user_id=test_user.id)
...
class TestUserSchemas:
    ...
    def test_user_create_invalid_email(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            UserCreate(name="John Doe", username="johndoe", email="invalid-email", password="SecurePass123!")
    ...
    def test_user_create_short_password(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            UserCreate(name="John Doe", username="johndoe", email="john@example.com", password="123")
...
class TestPostSchemas:
    ...
    def test_post_create_empty_title(self):
        ...
        with pytest.raises(ValidationError):
            PostCreate(title="", content="This is a test post content")

    def test_post_create_long_title(self):
        """Test title length validation."""
        with pytest.raises(ValidationError):
            PostCreate(title="x" * 101, content="This is a test post content")  # Exceeds max length
```
...
class TestUserCRUD:
    ...
    async def test_get_user_by_email(self, async_session, test_user):
        ...
        retrieved_user = await crud_users.get(db=async_session, email=test_user.email)
    ...
    async def test_update_user(self, async_session, test_user):
        ...
        updated_user = await crud_users.update(db=async_session, object=update_data, id=test_user.id)
    ...
    async def test_delete_user(self, async_session, test_user):
        ...
        deleted_user = await crud_users.get(db=async_session, id=test_user.id, is_deleted=True)
    ...
    async def test_get_multi_users(self, async_session):
        ...
        for i in range(5):
            user_data = UserCreate(
                name=f"User {i}", username=f"user{i}", email=f"user{i}@example.com", password="password123"
            )
            await crud_users.create(db=async_session, object=user_data)

        # Get users with pagination
        result = await crud_users.get_multi(db=async_session, offset=0, limit=3)
...
class TestPostCRUD:
    ...
    async def test_create_post(self, async_session, test_user):
        ...
        post = await crud_posts.create(db=async_session, object=post_data, created_by_user_id=test_user.id)
    ...
    async def test_get_posts_by_user(self, async_session, test_user):
        ...
        for i in range(3):
            post_data = PostCreate(title=f"Post {i}", content=f"Content {i}")
            await crud_posts.create(db=async_session, object=post_data, created_by_user_id=test_user.id)

        # Get posts by user
        result = await crud_posts.get_multi(db=async_session, created_by_user_id=test_user.id)
...
class TestUserAPI:
    ...
    async def test_update_user_authorized(self, async_client: AsyncClient, test_user, auth_headers):
        ...
        response = await async_client.patch(f"/api/v1/users/{test_user.id}", json=update_data, headers=auth_headers)
        assert response.status_code == 200
    ...
    async def test_update_user_unauthorized(self, async_client: AsyncClient, test_user):
        ...
        response = await async_client.patch(f"/api/v1/users/{test_user.id}", json=update_data)
    ...
    async def test_delete_user_forbidden(self, async_client: AsyncClient, test_user, auth_headers):
        ...
        response = await async_client.delete(f"/api/v1/users/{test_user.id}", headers=auth_headers)
