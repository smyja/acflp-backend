...
**Authentication**: Verifies user identity - confirms the user is who they claim to be **Authorization**: Determines user permissions - decides what the authenticated user can access
...
1. **Tiered Access**: Different user tiers have different capabilities and limits
1. **Contextual Authorization**: Permissions based on request context (rate limits, time-based access)
...
async def get_all_users(db: AsyncSession = Depends(async_get_db)) -> list[UserRead]:
    # Only superusers can access this endpoint
    users = await crud_users.get_multi(db=db, schema_to_select=UserRead, return_as_model=True)
    return users.data
```
...
async def get_my_posts(
    ...
    posts = await crud_posts.get_multi(
        db=db, created_by_user_id=current_user["id"], schema_to_select=PostRead, return_as_model=True
    )
    return posts.data
...
async def delete_post(
    post_id: int, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(async_get_db)
) -> dict[str, str]:
    # 1. Get the post
...
1. **Check Ownership**: Compare resource owner with current user
1. **Authorize or Deny**: Allow action if user owns resource, deny otherwise
...
async def create_post(
    post: PostCreate, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(async_get_db)
) -> PostRead:
    # Check rate limits based on user tier
    await check_rate_limit(resource="posts", user_id=current_user["id"], tier_id=current_user.get("tier_id"), db=db)

    # Create post with user association
    post_internal = PostCreateInternal(**post.model_dump(), created_by_user_id=current_user["id"])
...
```python
async def check_rate_limit(resource: str, user_id: int, tier_id: int | None, db: AsyncSession) -> None:
    ...
    recent_posts = await crud_posts.count(
        db=db, created_by_user_id=user_id, created_at__gte=datetime.utcnow() - timedelta(hours=24)
    )
...
async def update_post(
    ...
    updated_post = await crud_posts.update(db=db, object=post_updates, id=post_id)
    return updated_post
```
...
async def get_current_user(token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(async_get_db)) -> dict:
...
1. **Endpoint Dependencies**: FastAPI dependency injection for common patterns
1. **Business Logic**: Method-level permission validation
1. **Database**: Row-level security where applicable
...
async def update_user_tier(
    ...
    # 4. Update user tier
    await crud_users.update(db=db, object={"tier_id": tier_update.tier_id}, id=user_id)
...
async def log_authorization_event(user_id: int, action: str, resource: str, result: str, details: dict = None):
    """Log authorization events for security auditing."""
    security_logger.info(
        f"Authorization {result}: User {user_id} attempted {action} on {resource}",
        extra={"user_id": user_id, "action": action, "resource": resource, "result": result, "details": details or {}},
    )
...
async def delete_user_account(user_id: int, current_user: dict, db: AsyncSession):
    ...
    await log_authorization_event(
        user_id=current_user["id"], action="delete_account", resource=f"user:{user_id}", result="granted"
    )
...
async def get_organization_users(
    org_id: int, current_user: dict = Depends(get_current_user), db: AsyncSession = Depends(async_get_db)
) -> list[UserRead]:
    # Check if user belongs to organization
    membership = await crud_org_members.get(db=db, organization_id=org_id, user_id=current_user["id"])
    ...
    users = await crud_users.get_multi(db=db, organization_id=org_id, schema_to_select=UserRead, return_as_model=True)
...
# Usage in dependency
async def require_business_hours(current_user: dict = Depends(get_current_user)) -> dict:
...
def has_role(user: dict, required_role: Role) -> bool:
    ...
    role_hierarchy = {Role.USER: 0, Role.MODERATOR: 1, Role.ADMIN: 2, Role.SUPERUSER: 3}
...
def require_feature(feature_name: str):
    ...
    async def check_feature_access(
        ...
        if not await has_feature_access(current_user, feature_name, db):
            raise HTTPException(status_code=403, detail=f"Access to {feature_name} feature not available")
        return current_user
