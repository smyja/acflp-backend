...
class TestWriteTier:
    ...
    @pytest.mark.asyncio
    async def test_create_tier_success(self, mock_db, sample_tier_data, sample_tier_read):
        """Test successful tier creation."""
        tier_create = TierCreate(**sample_tier_data)
        request = Mock(spec=Request)
        
        with patch("app.api.v1.tiers.crud_tiers") as mock_crud:
            ...
            mock_crud.get.assert_called_once_with(db=mock_db, id=1, schema_to_select=TierRead)
    ...
    @pytest.mark.asyncio
    async def test_create_tier_not_found_after_creation(self, mock_db, sample_tier_data):
        ...
        with patch("app.api.v1.tiers.crud_tiers") as mock_crud:
            mock_crud.exists = AsyncMock(return_value=False)
            mock_crud.create = AsyncMock(return_value=Mock(id=1))
            mock_crud.get = AsyncMock(return_value=None)  # Tier not found after creation
...
class TestReadTiers:
    ...
    async def test_read_tiers_success(self, mock_db):
        ...
        request = Mock(spec=Request)
        mock_tiers_data = {"data": [{"id": 1, "name": "free"}, {"id": 2, "name": "premium"}], "count": 2}
        
        with patch("app.api.v1.tiers.crud_tiers") as mock_crud:
            mock_crud.get_multi = AsyncMock(return_value=mock_tiers_data)
            
            with patch("app.api.v1.tiers.paginated_response") as mock_paginated:
                expected_response = {
                ...
                mock_crud.get_multi.assert_called_once_with(db=mock_db, offset=0, limit=10)
    ...
    async def test_read_tiers_with_pagination(self, mock_db):
        ...
        with patch("app.api.v1.tiers.crud_tiers") as mock_crud:
            ...
            with patch("app.api.v1.tiers.paginated_response") as mock_paginated:
                with patch("app.api.v1.tiers.compute_offset") as mock_offset:
                    mock_offset.return_value = 20
                    mock_paginated.return_value = {"data": [], "pagination": {}}
                    
                    result = await read_tiers(request, mock_db, page=3, items_per_page=20)
                    
                    mock_offset.assert_called_once_with(3, 20)
                    mock_crud.get_multi.assert_called_once_with(db=mock_db, offset=20, limit=20)
...
class TestTierPermissions:
    ...
    async def test_read_operations_no_auth_required(self, mock_db, sample_tier_read):
        ...
        with patch("app.api.v1.tiers.crud_tiers") as mock_crud:
            ...
            mock_crud.get_multi = AsyncMock(return_value={"data": [sample_tier_read], "count": 1})
            with patch("app.api.v1.tiers.paginated_response") as mock_paginated:
                mock_paginated.return_value = {"data": [sample_tier_read], "pagination": {}}
