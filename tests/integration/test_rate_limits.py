...
class TestWriteRateLimit:
    ...
    async def test_create_rate_limit_success(self, mock_db, sample_tier_read):
        ...
        with patch("app.api.v1.rate_limits.crud_tiers") as mock_crud_tiers:
            with patch("app.api.v1.rate_limits.crud_rate_limits") as mock_crud_rate_limits:
                ...
                result = await write_rate_limit(request, tier_name, rate_limit_create, mock_db)
                
                assert result == mock_rate_limit_read
                mock_crud_tiers.get.assert_called_once_with(db=mock_db, name=tier_name, schema_to_select=TierRead)
    ...
    @pytest.mark.asyncio
    async def test_create_rate_limit_not_found_after_creation(self, mock_db, sample_tier_read):
        ...
        with patch("app.api.v1.rate_limits.crud_tiers") as mock_crud_tiers:
            with patch("app.api.v1.rate_limits.crud_rate_limits") as mock_crud_rate_limits:
                ...
                with pytest.raises(NotFoundException, match="Created rate limit not found"):
                    await write_rate_limit(request, tier_name, rate_limit_create, mock_db)
...
class TestReadRateLimits:
    ...
    async def test_read_rate_limits_success(self, mock_db, sample_tier_read):
        ...
        with patch("app.api.v1.rate_limits.crud_tiers") as mock_crud_tiers:
            with patch("app.api.v1.rate_limits.crud_rate_limits") as mock_crud_rate_limits:
                mock_crud_tiers.get = AsyncMock(return_value=sample_tier_read)
                mock_crud_rate_limits.get_multi = AsyncMock(return_value=mock_rate_limits_data)
                
                with patch("app.api.v1.rate_limits.paginated_response") as mock_paginated:
                    expected_response = {
                    ...
                    result = await read_rate_limits(request, tier_name, mock_db, page=1, items_per_page=10)
    ...
    async def test_read_rate_limit_success(self, mock_db, sample_tier_read):
        ...
        with patch("app.api.v1.rate_limits.crud_tiers") as mock_crud_tiers:
            with patch("app.api.v1.rate_limits.crud_rate_limits") as mock_crud_rate_limits:
                ...
                result = await read_rate_limit(request, tier_name, rate_limit_id, mock_db)
                
                assert result == mock_rate_limit_read
                mock_crud_tiers.get.assert_called_once_with(db=mock_db, name=tier_name, schema_to_select=TierRead)
                mock_crud_rate_limits.get.assert_called_once_with(
                    db=mock_db, tier_id=sample_tier_read.id, id=rate_limit_id, schema_to_select=RateLimitRead
                )
...
class TestPatchRateLimit:
    ...
    async def test_patch_rate_limit_success(self, mock_db, sample_tier_read):
        ...
        with patch("app.api.v1.rate_limits.crud_tiers") as mock_crud_tiers:
            with patch("app.api.v1.rate_limits.crud_rate_limits") as mock_crud_rate_limits:
                ...
                result = await patch_rate_limit(request, tier_name, rate_limit_id, rate_limit_update, mock_db)
                ...
                mock_crud_rate_limits.update.assert_called_once_with(db=mock_db, object=rate_limit_update, id=rate_limit_id)
    ...
    async def test_patch_rate_limit_tier_not_found(self, mock_db):
        ...
        with patch("app.api.v1.rate_limits.crud_tiers") as mock_crud_tiers:
            ...
            with pytest.raises(NotFoundException, match="Tier not found"):
                await patch_rate_limit(request, tier_name, rate_limit_id, rate_limit_update, mock_db)
    ...
    async def test_erase_rate_limit_success(self, mock_db, sample_tier_read):
        ...
        with patch("app.api.v1.rate_limits.crud_tiers") as mock_crud_tiers:
            with patch("app.api.v1.rate_limits.crud_rate_limits") as mock_crud_rate_limits:
                ...
                result = await erase_rate_limit(request, tier_name, rate_limit_id, mock_db)
                ...
                mock_crud_rate_limits.delete.assert_called_once_with(db=mock_db, id=rate_limit_id)
    ...
    async def test_erase_rate_limit_database_error(self, mock_db, sample_tier_read):
        ...
        with patch("app.api.v1.rate_limits.crud_tiers") as mock_crud_tiers:
            with patch("app.api.v1.rate_limits.crud_rate_limits") as mock_crud_rate_limits:
                mock_crud_tiers.get = AsyncMock(return_value=sample_tier_read)
                mock_crud_rate_limits.get = AsyncMock(return_value=mock_rate_limit_read)
                mock_crud_rate_limits.delete = AsyncMock(side_effect=Exception("Database error"))
    ...
    async def test_read_operations_no_auth_required(self, mock_db, sample_tier_read):
        ...
        with patch("app.api.v1.rate_limits.crud_tiers") as mock_crud_tiers:
            with patch("app.api.v1.rate_limits.crud_rate_limits") as mock_crud_rate_limits:
                ...
                mock_crud_rate_limits.get_multi = AsyncMock(return_value={"data": [mock_rate_limit_read], "count": 1})
                with patch("app.api.v1.rate_limits.paginated_response") as mock_paginated:
                    mock_paginated.return_value = {"data": [mock_rate_limit_read], "pagination": {}}
